...
const StartMenu: React.FC<Props> = ({ user, apps, systemAppIds, onLaunch, isOpen, integrity }) => {
    ...
    const fetchWeather = async () => {
      try {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: "Provide current weather for SF. 5 words max.",
    ...
    >
      <div className="p-10 pb-6 bg-gradient-to-br from-white/5 to-transparent">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => window.location.reload()} className="w-10 h-10 rounded-full hover:bg-white/10 flex items-center justify-center text-white/20 hover:text-red-400 transition-all border border-transparent hover:border-red-500/20">
        ...
        <div className="relative group">
          ...
          <input autoFocus className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-14 pr-6 text-sm text-white outline-none focus:border-white/20 transition-all placeholder:text-white/20" placeholder="Type to search Curium..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 pb-8 space-y-10 scrollbar-hide">
        {mostUsed.length > 0 && !search && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-700">
            ...
            <div className="grid grid-cols-4 gap-4">
              {mostUsed.map(app => {
                const isSystemApp = systemAppIds.includes(app.id);
                const shouldShowIcon = !isSystemApp || (isSystemApp && integrity.hasIcons);
                return (
                  <button key={app.id} onClick={() => onLaunch(app.id)} className="flex flex-col items-center gap-3 p-3 rounded-2xl hover:bg-white/5 active:scale-95 transition-all group">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-1000">
          <h3 className="text-[10px] uppercase tracking-[0.25em] text-white/30 font-black mb-5 px-1">{search ? 'Search Results' : 'System Library'}</h3>
          <div className="grid grid-cols-1 gap-1.5">
            {filteredApps.map(app => {
              const isSystemApp = systemAppIds.includes(app.id);
              const shouldShowIcon = !isSystemApp || (isSystemApp && integrity.hasIcons);
              return (
                <button key={app.id} onClick={() => onLaunch(app.id)} className="w-full flex items-center gap-5 p-4 rounded-2xl hover:bg-white/5 active:scale-[0.98] transition-all group border border-transparent hover:border-white/5">
      ...
      <div className="p-6 bg-white/[0.02] border-t border-white/5 flex items-center justify-between px-10">
        <button onClick={() => onLaunch('settings')} className="text-[11px] text-white/40 hover:text-white flex items-center gap-2.5 transition-colors font-bold tracking-tight">
          <i className="fas fa-cog text-xs"></i> System Management
        </button>
